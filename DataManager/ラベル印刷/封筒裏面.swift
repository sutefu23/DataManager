...
\u{25FC}研磨材・タワシ・ペーパー等、表面に傷を付ける恐れのあるものは、絶対に使用しないでください。
\u{25FC}溶解性(アルカリ性・酸性の洗剤など)・揮発性(シンナー・アルコールなど)の薬品は絶対に使用しないで下さい。
\u{25FC}塗装面(真鍮・銅・メッキ製品への透明クリアも含む)に、テープ類は貼らないで下さい。
...
\u{25FC}まずは、製品に付着したチリや埃をやわらかい布で落とした後、水を含ませた布で、
　やさしくなでる様に表面の目に沿って拭いて下さい。
　その後は、必ず柔らかい布などを使用し、同じ要領で乾拭きを行って下さい。
\u{25FC}浮かして取り付けてある商品は、強い力を加えないように作業を行ってください。
\u{25FC}製品によっては鋭利な部分があるので、お手入れの際は十分にお気をつけください。
""", fontSize: normalFont, bold: false, color: .black, nooffset: true, lineSpace: lineSpace)
...
\u{25FC}ステンレス製品は錆びにくい素材であり、決して錆びない素材ではありません。
　工場現場や線路のそばなど、飛んできた鉄粉や粉塵の付着によって、もらい錆を受けます。
\u{25FC}銅・真鍮製品は必ず変色する素材です。弊社では変色を遅らせる為に透明クリア塗装を施しております。

...
class TestUseItemRecord: XCTestCase {
    ...
    func testRegist() {
        let use = 資材使用記録型(登録日時: date1, 伝票番号: order1, 工程: .フォーミング, 作業者: .関雄也, 図番: item1.図番, 表示名: item1.標準表示名, 単価: item1.単価, 用途: "天板", 使用量: "100x100 3枚", 単位量: (100.0*100.0)/(1219.0*2438.0), 単位数: 3, 金額: 782, 印刷対象: .全て)
